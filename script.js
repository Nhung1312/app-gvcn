...
function initData() {
    ...
    if (!v4Data) {
        ...
        v4Data = {
            ...
            students: v3Data ? v3Data.students : [], 
            attendance: v3Data ? v3Data.attendance : {},
            behaviorTags: defaultTags, 
            behaviorRecords: v3Data ? v3Data.behaviorRecords||[] : [], 
            tasks: v3Data ? v3Data.tasks||[] : [], 
            notifications: v3Data ? v3Data.notifications||[] : [], 
            documents: v3Data ? v3Data.documents||[] : [],
            activityGroups: [],
        ...
        if (!v4Data.activityGroups) v4Data.activityGroups = [];
...
window.resetData = function() { 
    if(confirm("XÓA TOÀN BỘ CSDL CỤC BỘ? LƯU Ý: Thao tác này chỉ xóa bộ nhớ tạm, dữ liệu mây vẫn còn.")) { 
        localStorage.removeItem('gvcnData_v4'); 
        location.reload(); 
    } 
};
...
window.updateDashboardInfo = function() {
    const s = appData.settings;
    document.getElementById('dash-teacher').innerText = s.teacherName; 
    document.getElementById('dash-class').innerText = s.className; 
    document.getElementById('dash-year').innerText = s.year;
    
    const totalStu = appData.students.length; 
    document.getElementById('dash-count').innerText = totalStu + " HS"; 
    document.getElementById('stat-total').innerText = totalStu;
            ...
            try {
                ...
                rows.forEach(r => {
                    if(currentImportType === 'tkb' && (r['Thứ'] || r['Thu']) && (r['Tiết'] || r['Tiet']) && r['Lớp']) {
                        ...
                        let n = String(r['Nội dung'] || r['Noi dung'] || r['Tên bài'] || r['Ten bài'] || r['Chủ đề'] || r['Tên bài học / Chuyên đề'] || '');
...
window.renderStudents = function() {
    ...
    if(filtered.length === 0) { list.innerHTML = '<div class="empty-state"><h4>Chưa có học sinh nào trong lớp!</h4><p>Hãy bấm Nhập file hoặc Thêm học sinh.</p></div>'; return; }
...
window.saveStudent = function() {
    ...
    const obj = { 
        ...
        team: parseInt(document.getElementById('stu-team').value) || 1,
        dob: document.getElementById('stu-dob').value, 
        phone: document.getElementById('stu-phone').value 
    };
    if(id) { 
        appData.students[appData.students.findIndex(s => s.id == id)] = obj; 
        window.showToast("Đã cập nhật!"); 
    } else { 
        appData.students.push(obj); 
        window.showToast("Đã thêm!"); 
    }
    window.saveData(); 
    window.renderStudents(); 
...
// ================= CHIA TỔ CỐ ĐỊNH & NHÓM HOẠT ĐỘNG (GVCN) =================
...
window.autoDivideFixedTeams = function() {
    ...
    if(!confirm("Chia đều học sinh hiện tại vào 4 Tổ cố định? (Bạn có thể đổi tổ từng em sau khi chia)")) return;
...
window.generateActivityGroups = function() {
    ...
    if (divideType === 'byGroupCount') {
        numGroups = Math.max(2, Math.min(appData.students.length, paramVal));
    } else {
        numGroups = Math.max(2, Math.ceil(appData.students.length / paramVal));
    }
    ...
    window.saveData();
    window.renderGroupsView();
    window.showToast(`🎉 Đã chia ${numGroups} nhóm thảo luận!`);
};
...
window.renderGroupsView = function() {
    ...
    if (currentGroupSubMode === 'teams') {
        if(!appData.students || appData.students.length === 0) {
            container.innerHTML = '<div style="text-align:center; padding:30px; color:var(--text-muted);">Lớp chưa có học sinh.</div>';
            return;
        }
                ...
                membersHtml += `
                    <div class="group-member-row">
                        <div style="display: flex; align-items: center; gap: 8px;">
                            ...
                            <div class="avatar-circle-mini ${m.gender === 'Nữ' ? 'female' : ''}">${initials}</div>
                            <div>
                                <strong style="font-size: 0.9rem; color: var(--text-main);">${m.name}</strong>
                                <small style="display: block; color: var(--text-muted); font-size: 0.75rem;">${m.gender} • Sinh: ${m.dob || '--'}</small>
                            </div>
                        </div>
            ...
            grp.members.forEach((m, mIdx) => {
                ...
                membersHtml += `
                    <div class="group-member-row">
                        ...
                        <button class="btn-icon-mini text-red" title="Chuyển sang nhóm khác" onclick="moveMemberOutOfActivityGroup(${gIdx}, ${mIdx})"><i class="fas fa-arrows-alt-h"></i></button>
                    </div>
                `;
            });
...
window.confirmImport = function() { 
    ...
    window.showToast(`Đã nhập ${c} HS!`); 
};

window.exportExcel = function() { 
    if(appData.students.length === 0) return window.showToast("Lớp trống!", "error"); 
...
window.renderAttendance = function() {
    ...
    appData.students.forEach((stu, index) => {
        ...
        list.innerHTML += `<div class="list-item"><div class="list-item-info"><strong>${index + 1}. ${stu.name}</strong><small>Tổ ${stu.team || 1}</small></div><div class="attendance-opts"><button class="att-btn ${status === 'present' ? 'active' : ''}" data-status="present" onclick="setAtt(this, ${stu.id}, 'present')"><i class="fas fa-check"></i></button><button class="att-btn ${status === 'excused' ? 'active' : ''}" data-status="excused" onclick="setAtt(this, ${stu.id}, 'excused')"><i class="fas fa-exclamation"></i></button><button class="att-btn ${status === 'unexcused' ? 'active' : ''}" data-status="unexcused" onclick="setAtt(this, ${stu.id}, 'unexcused')"><i class="fas fa-times"></i></button></div></div>`;
    });
...
window.renderDisciplineStudents = function() {
    ...
    appData.students.filter(s => s.name.toLowerCase().includes(txt)).forEach(stu => {
        ...
        list.innerHTML += `<div class="list-item" style="cursor:pointer;" onclick="openRecordBehavior(${stu.id}, '${stu.name}')"><div class="list-item-info"><strong>${stu.name}</strong><small>Tổ ${stu.team || 1}</small></div><div><span style="color:${color}; font-weight:800; font-size:1.1rem;">${pts > 0 ? '+'+pts : pts} đ</span> <i class="fas fa-chevron-right text-muted ml-2"></i></div></div>`;
    });
};
...
window.confirmTagRecord = function(tagId) { 
    const tag = appData.behaviorTags.find(t => t.id === tagId); if(!tag) return; 
    document.getElementById('conf-tag-id').value = tag.id; 
    document.getElementById('conf-stu-id').value = currentDiscStuId; 
...
window.autoCreateBehavior = function(stuId, tag, date, note) { 
    if(!appData.behaviorRecords.find(r => r.studentId === stuId && r.date === date && r.tagId === tag.id && r.note.includes("Hệ thống"))) { 
        appData.behaviorRecords.push({ id: Date.now() + Math.random(), studentId: stuId, tagId: tag.id, snapshotName: tag.name, snapshotPoints: tag.currentPoints, type: tag.type, date: date, time: formatDateTime(), note: note }); 
        window.saveData(); 
    } 
};
...
const CURRENT_APP_VERSION = 'v4.2_gvcn_complete_teams_reset';
